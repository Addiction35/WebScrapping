...
async def scrape_product_data(session, product_info):
    ...
    try:
        product_name = soup.select_one(config['data_selectors'].get('product_name'))
        sku = soup.select_one(config['data_selectors'].get('sku'))
        stock_level = soup.select_one(config['data_selectors'].get('stock_level'))
        wholesale_price = soup.select_one(config['data_selectors'].get('wholesale_price'))
        product_price = soup.select_one(config['data_selectors'].get('product_price'))
        ...
        if stock_level:
            stock_level = stock_level.get_text(strip=True)
        if wholesale_price:
            wholesale_price = extract_price(wholesale_price)
        if product_price:
        ...
        print(f"Extracted data for {url}: {product_data}")
