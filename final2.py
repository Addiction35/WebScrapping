...
async def scrape_product_data(session, product_info):
    ...
    scraperapi_key = product_info['scraperapi_key']
    ...
    try:
        product_data = {
            ...
            'wholesale_price': soup.select_one(config['data_selectors']['wholesale_price']).text if config['data_selectors']['wholesale_price'] and soup.select_one(config['data_selectors']['wholesale_price']) else None,
            'product_price': soup.select_one(config['data_selectors']['product_price']).text if config['data_selectors']['product_price'] and soup.select_one(config['data_selectors']['product_price']) else None,
        }

        # Convert wholesale_price and product_price to numeric values if they exist
        if product_data['wholesale_price'] is not None:
            product_data['wholesale_price'] = float(product_data['wholesale_price'].replace(',', '').replace('$', '').strip())
        if product_data['product_price'] is not None:
            product_data['product_price'] = float(product_data['product_price'].replace(',', '').replace('$', '').strip())

    except Exception as e:
        return None  # Return None if scraping fails

    return product_data if any(product_data.values()) else None  # Return None if all values are None
    ...
    async with aiohttp.ClientSession() as session:
        async with aiofiles.open("scraped_products.json", "w") as output_file:
            ...
            for product_info in product_urls:
                ...
                task = scrape_and_write_product_data(session, product_info, domain_data)
                tasks.append(task)
                processed_urls.add(url)  # Mark URL as processed

            product_entries = await asyncio.gather(*tasks)
            product_entries = [entry for entry in product_entries if entry is not None]  # Filter out None entries
